...
func _ParseStringPacket(strPacket: String) -> void:
	...
	match(net_code):
		...
		EGlobalEnums.NETCODE.CHANGE_PLAYER:
			_packet = {"netcode": net_code, "current_player": int(strPacketBlocks[1]) as EGlobalEnums.PLAYER_TYPE, "state_p1": str(strPacketBlocks[2]), "state_p2": str(strPacketBlocks[3]), "min": int(strPacketBlocks[5]), "sec": int(strPacketBlocks[6]), "nickname": strPacketBlocks[7]};
